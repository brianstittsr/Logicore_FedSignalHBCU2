...
export default function AIPageDesignerPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
      ...
      {/* Design Wizard Dialog */}
      <Dialog open={showWizard} onOpenChange={setShowWizard}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle>Page Design Wizard</DialogTitle>
            <DialogDescription>Follow these steps to create a new page design</DialogDescription>
          </DialogHeader>
          <div className="flex items-center justify-between px-4 py-2">
            {[1,2,3,4,5,6,7].map((step,index)=>(<div key={step} className="flex items-center"><div className={"w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium "+(index<wizardStep?"bg-green-500 text-white":index===wizardStep?"bg-blue-500 text-white":"bg-gray-200 text-gray-500")}>{index<wizardStep?<Check className="w-4 h-4"/>:step}</div>{index<6&&<div className={"w-8 h-0.5 "+(index<wizardStep?"bg-green-500":"bg-gray-200")}/>}</div>))}
          </div>
          <div className="flex-1 overflow-y-auto p-4">
            {wizardStep===0&&(<div className="space-y-4"><h3 className="font-semibold text-lg">Select Page Type</h3><p className="text-gray-500 text-sm">Choose the type of page</p><div className="grid grid-cols-2 gap-4 mt-4">{[{id:"landing",name:"Landing Page",icon:Rocket,desc:"Event page"},{id:"marketing",name:"Marketing Page",icon:TrendingUp,desc:"Product showcase"},{id:"content",name:"Content Page",icon:FileText,desc:"Blog/article"},{id:"form",name:"Form Page",icon:Square,desc:"Contact form"}].map(t=>(<div key={t.id} onClick={()=>setWizardData(p=>({...p,pageType:t.id}))} className={"p-4 border-2 rounded-lg cursor-pointer "+(wizardData.pageType===t.id?"border-blue-500 bg-blue-50":"border-gray-200 hover:border-gray-300")}><t.icon className="w-8 h-8 mb-2 text-blue-500"/><h4 className="font-medium">{t.name}</h4><p className="text-sm text-gray-500">{t.desc}</p></div>))}</div></div>)}
            {wizardStep===1&&(<div className="space-y-4"><h3 className="font-semibold text-lg">Basic Information</h3><div className="space-y-4 mt-4"><div><Label>Page Name *</Label><Input value={wizardData.pageName} onChange={e=>setWizardData(p=>({...p,pageName:e.target.value}))} placeholder="Page name"/></div><div><Label>URL Slug *</Label><div className="flex items-center gap-2"><span className="text-gray-500 text-sm">/landing/</span><Input value={wizardData.pageSlug} onChange={e=>setWizardData(p=>({...p,pageSlug:e.target.value.toLowerCase().replace(/[^a-z0-9-]/g,"-")}))} placeholder="url-slug" className="flex-1"/></div></div></div></div>)}
            {wizardStep===2&&(<div className="space-y-4"><h3 className="font-semibold text-lg">Choose Layout</h3><div className="grid grid-cols-2 gap-4 mt-4">{[{id:"single-column",name:"Single Column"},{id:"two-column",name:"Two Column"},{id:"full-width",name:"Full Width"},{id:"centered",name:"Centered"}].map(l=>(<div key={l.id} onClick={()=>setWizardData(p=>({...p,layout:l.id}))} className={"p-4 border-2 rounded-lg cursor-pointer text-center "+(wizardData.layout===l.id?"border-blue-500 bg-blue-50":"border-gray-200")}><div className="w-full h-16 bg-gray-100 rounded mb-2 flex items-center justify-center"><Layout className="w-6 h-6 text-gray-400"/></div><h4 className="font-medium">{l.name}</h4></div>))}</div></div>)}
            {wizardStep===3&&(<div className="space-y-4"><h3 className="font-semibold text-lg">Add Sections</h3><div className="grid grid-cols-2 gap-3 mt-4">{["hero","features","testimonials","pricing","faq","cta","gallery","team","contact","stats"].map(s=>(<div key={s} onClick={()=>setWizardData(p=>({...p,sections:p.sections.includes(s)?p.sections.filter(x=>x!==s):[...p.sections,s]}))} className={"p-3 border-2 rounded-lg cursor-pointer flex items-center gap-3 "+(wizardData.sections.includes(s)?"border-blue-500 bg-blue-50":"border-gray-200")}><div className={"w-5 h-5 rounded border-2 flex items-center justify-center "+(wizardData.sections.includes(s)?"border-blue-500 bg-blue-500":"border-gray-300")}>{wizardData.sections.includes(s)&&<Check className="w-3 h-3 text-white"/>}</div><span className="font-medium text-sm capitalize">{s}</span></div>))}</div></div>)}
            {wizardStep===4&&(<div className="space-y-4"><h3 className="font-semibold text-lg">Add Content</h3><div className="space-y-4 mt-4"><div><Label>Hero Title</Label><Input value={wizardData.heroTitle} onChange={e=>setWizardData(p=>({...p,heroTitle:e.target.value}))} placeholder="Main headline"/></div><div><Label>Hero Subtitle</Label><Textarea value={wizardData.heroSubtitle} onChange={e=>setWizardData(p=>({...p,heroSubtitle:e.target.value}))} placeholder="Supporting text" rows={3}/></div><div className="p-4 bg-gray-50 rounded-lg text-center"><h2 className="text-xl font-bold">{wizardData.heroTitle||"Title"}</h2><p className="text-gray-500 mt-2">{wizardData.heroSubtitle||"Subtitle"}</p></div></div></div>)}
            {wizardStep===5&&(<div className="space-y-4"><h3 className="font-semibold text-lg">Customize Styling</h3><div className="space-y-4 mt-4"><div><Label>Primary Color</Label><div className="flex items-center gap-3"><input type="color" value={wizardData.primaryColor} onChange={e=>setWizardData(p=>({...p,primaryColor:e.target.value}))} className="w-12 h-10 rounded cursor-pointer"/><Input value={wizardData.primaryColor} onChange={e=>setWizardData(p=>({...p,primaryColor:e.target.value}))} className="w-32"/></div></div><div><Label>Secondary Color</Label><div className="flex items-center gap-3"><input type="color" value={wizardData.secondaryColor} onChange={e=>setWizardData(p=>({...p,secondaryColor:e.target.value}))} className="w-12 h-10 rounded cursor-pointer"/><Input value={wizardData.secondaryColor} onChange={e=>setWizardData(p=>({...p,secondaryColor:e.target.value}))} className="w-32"/></div></div><div className="flex gap-4 mt-4"><Button style={{backgroundColor:wizardData.primaryColor}}>Primary</Button><Button variant="outline" style={{borderColor:wizardData.secondaryColor,color:wizardData.secondaryColor}}>Secondary</Button></div></div></div>)}
            {wizardStep===6&&(<div className="space-y-4"><h3 className="font-semibold text-lg">SEO & Publish</h3><div className="space-y-4 mt-4"><div><Label>SEO Title</Label><Input value={wizardData.seoTitle} onChange={e=>setWizardData(p=>({...p,seoTitle:e.target.value}))} placeholder="SEO title"/><p className="text-xs text-gray-400 mt-1">{wizardData.seoTitle.length}/60</p></div><div><Label>Meta Description</Label><Textarea value={wizardData.seoDescription} onChange={e=>setWizardData(p=>({...p,seoDescription:e.target.value}))} placeholder="Description" rows={2}/><p className="text-xs text-gray-400 mt-1">{wizardData.seoDescription.length}/160</p></div><div className="p-4 bg-green-50 border border-green-200 rounded-lg"><h4 className="font-medium text-green-800 flex items-center gap-2"><CheckCircle className="w-5 h-5"/>Ready to Publish</h4><p className="text-sm text-green-700 mt-1">URL: <strong>/landing/{wizardData.pageSlug||"page"}</strong></p></div></div></div>)}
          </div>
          <DialogFooter className="border-t pt-4">
            <Button variant="outline" onClick={()=>{if(wizardStep===0){setShowWizard(false);}else{setWizardStep(p=>p-1);}}}>{wizardStep===0?"Cancel":"Previous"}</Button>
            <Button onClick={()=>{if(wizardStep<6){setWizardStep(p=>p+1);}else{const np:LandingPageConfig={id:"lp-"+Date.now(),name:wizardData.pageName,slug:wizardData.pageSlug,published:true,elements:[],buttons:[],seoTitle:wizardData.seoTitle||wizardData.pageName,seoDescription:wizardData.seoDescription};setLandingPages(p=>[...p,np]);setCurrentLandingPage(np);setShowWizard(false);setWizardStep(0);setWizardData({pageType:"landing",pageName:"",pageSlug:"",layout:"single-column",sections:[],heroTitle:"",heroSubtitle:"",primaryColor:"#2563EB",secondaryColor:"#10B981",seoTitle:"",seoDescription:""});}}}>{wizardStep===6?"Create Page":"Next"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
